...
int montecarlo_word(char * random_word) {
    ...
    int i=0, wc=0;
    int rand_char;
    //Aquí está la función a la que llama. El deseeder es una función de impress.cpp.
    //getchar();
    if(initiated == 0)
    {
        srand(deseeder());   
    ...
    //printf("Length %d token %Ld word %s\n", i+1, token, random_word); 
    return i+1;
} 
...
int random_word_block(char * buf) {
    ...
    while(buf_len < MY_BLOCK_SIZE) {
        if(IMP_input->Flag[sf_wordfreq]==1) { // Sigurd Et. Al.
            //Entra aquí por un parámetro en el fichero de configuración que le ponemos nostros.
            word_len = montecarlo_word(random_word);
        }
