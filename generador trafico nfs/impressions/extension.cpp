...
long double make_generic_file(char * filepath, long double size, int depth, FILE * fp_log) {
    ...
    IMP_input->Flag[sf_ext] = 16;
    //printf("%d \n",IMP_input->Flag[sf_ext]);
    ...
    {
       // printf("%s\n",ext );
        sprintf(full_filepath, "%s.pdf", filepath,ext); //extension_array[ext_number]);
        //printf("%s\n", full_filepath);
    }
    ...
    */
    if(IMP_input->Actualfilecreation == 1) {
        ...
        switch(ext_number) { 
            ...
            case PDF:
                //printf("%s\n", full_filepath);
                make_ascii_file(full_filepath, size, 16);
...
int make_ascii_file(char * filepath, long double size, int extension_num) {
    ...
    if((fd = pos_creat(filepath, mode)) <0) {
            ...
            printf("Error: Unable to create file= %s %d\n", filepath, errno);
    }
    else { // write to file with size
