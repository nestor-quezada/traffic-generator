...
/* ****************************************************
   ...
   **************************************************** */
int deseeder() {
    //A ver, en el fichero de configuración le ponemos un número X de semillas (por defecto 10), que utiliza para la
    //genneración del tamaño de los archivos. Para la generación de las palabras, coge las diez y luego cuando ya ha cogido
    //todas y se queda sin ninguna empieza a coger el instante temporal.
...
int init_files_dirs_for_capacity() {
    ...
    if(IMP_input->FSused > 0) {
        if (IMP_input->Numfiles == -1)
            IMP_input->Numfiles = lrint(IMP_input->FSused/global_mean);
        IMP_input->Numdirs  = lrint((IMP_input->FSused/global_mean)/IMP_input->mean_files_per_dir);
    }
    return 1;
}
...
int main(int argc, char * argv[]) {
    ...
    if(ACTUAL_LOG_CREATION) {
        ...
        sprintf(logfilename, "%s", IMP_input->Actuallogfile);
    ...
    print_debug1(1, "Num files: %i, Num dirs = %i \n", IMP_input->Numfiles, IMP_input->Numdirs);
    ...
    */
    ...
    for (int i=0; i<IMP_input->Numfiles; i++) {
        ...
        //Línea para que todos los ficheros ocupen lo mínimo posible. Para el simulador de tráfico SMB (Nestor).
        data1 = (long double) (1/1024/1024/1024);
        //------------------------------------------------
       ...
       *(filearray+ NUM_FILE_ATTRIBS*i + PARENT_DIR)=data2;
        ...
        //double fileSize = make_generic_file(parent_path,data1, depth, fp_log);
        //printf("%s\n", parent_path);
